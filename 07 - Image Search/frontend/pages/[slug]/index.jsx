...
function Index() {
  ...
  const category = useMemo(() => cat.find(item => item.slug.toLowerCase()==slug?.toLowerCase()), [cat, slug])
  ...
  useEffect(() => {
   ...
   const searchData = async (e) => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:8000',{params:{cat:category.id, q: search}} );
    ...
    <div className='flex flex-col gap-4 h-full w-full p-4'>
      ...
      <div className='flex gap-4 '>
        ...
        <div className='flex flex-col gap-2 bg-white rounded-md p-4 basis-56'>
          ...
          className={`px-4 py-1 rounded-lg hover:bg-slate-400 cursor-pointer `}
          >
            {/* <a > */}
          ...
          className={`px-4 py-1 rounded-lg hover:bg-slate-400 cursor-pointer ${slug == cati.slug ? 'bg-rose-300' : ''}`}
          >
            {/* <a > */}
